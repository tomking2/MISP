...
class JSONConverterTool
{
    ...
    public static function convert($event, $isSiteAdmin=false, $raw = false)
    {
        ...
        if (isset($event['Event']['Attribute'])) {
            $event['Event']['Attribute'] = self::__cleanAttributes($event['Event']['Attribute'], $tempSightings, $event['Event']);
        }
        if (isset($event['Event']['Object'])) {
            $event['Event']['Object'] = self::__cleanObjects($event['Event']['Object'], $tempSightings, $event['Event']);
        }
        unset($tempSightings);
    ...
    private static function __cleanAttributes($attributes, $tempSightings = array(), $event = array())
    {
        ...
        foreach ($attributes as $key => $attribute) {
            ...
            if (isset($event['RelatedAttribute'][$attribute['id']])) {
                $attributes[$key]['RelatedAttribute'] = $event['RelatedAttribute'][$attribute['id']];
    ...
    private static function __cleanObjects($objects, $tempSightings = array(), $event)
    {
        foreach ($objects as $k => $object) {
            if (!empty($object['Attribute'])) {
                $objects[$k]['Attribute'] = self::__cleanAttributes($object['Attribute'], $tempSightings, $event);
            } else {
                unset($objects[$k]);
            }
        }
